...
We alluded to intuition that, in large systems, the product of microstates and Boltzmann factor is strongly peaked. The typical states of a large system lie on the peak, where Helmholtz free energy $F=E_{S}-T_{R} S_{S}$ is minimized. Note that in this thermodynamic limit, the energy $E_{S}$ of the system is deterministic, and mean adn median energy are identical. Not so in smaller systems where energies show appreciable fluctuations. Yet, we can easily derive a precise minimization principle in small systems if we define the system entropy as the Gibbs entropy
...
$$
\begin{aligned}
Z & =\frac 1{N!}\prod_{i}\int \frac{d \vec{p}_{i} d \vec{q}_{i}}{h^3} e^{-\beta \vec{p}_{i}^{2} / 2 m-U} \\
